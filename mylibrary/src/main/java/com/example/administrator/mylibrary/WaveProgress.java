...
public class WaveProgress extends View {
	...
	private void initLevelAnimation() {
		ObjectAnimator waveShiftAnim = ObjectAnimator.ofFloat(this, "waveLevel",waveLevel, progress/100f);
		waveShiftAnim.setDuration(1000);
		waveShiftAnim.setInterpolator(new LinearInterpolator());
		mAnimatorSet = new AnimatorSet();
		mAnimatorSet.play(waveShiftAnim);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		paint.setColor(adjustAlpha(mWaveColor, DEFAULT_ALPHA));
		final int wave2Shift = (int) (width / 4);
		for (int beginX = 0; beginX < endX; beginX++) {
			canvas.drawLine(beginX, pointY[(beginX + wave2Shift) % endX], beginX, endY, paint);
		}
		paint.setColor(adjustAlpha(mWaveColor, 0.1f));
		int wave3Shift = (int) (width / 6);
		for (int beginX = 0; beginX < endX; beginX++) {
